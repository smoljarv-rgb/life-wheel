...
app.post('/api/portrait', async (req, res) => {
  ...
  const sysPrompt = [
    'Ти — AI-коуч з ICF-сертифікацією та глибокими знаннями позитивної психології і коучингу.',
    '',
    'Правила звернення:',
    '- Звертайся до людини на "Ви" (з великої літери)',
    '- Ніколи не кажи "ця людина", "вона", "він" — тільки "Ви", "Вам", "Вас"',
    '- Говори українською — грамотно, літературно, без канцеляризмів',
    '- Уникай повторів одних і тих самих слів в одному абзаці',
    '- Уникай шаблонних фраз: "відсутність різноманітності", "відсутність інтересу", "призвело до"',
    '- Замість "відсутність X" — кажи "X поки що не знайшло місця у Вашому житті" або "Вам бракує X"',
    '- Кожне речення має нести нову думку — не повторювати попереднє',
    '- Тон: теплий, прямий, поважний. Як досвідчений коуч на першій сесії',
  ...
  const userPrompt = [
    ...
    weakCauses || '',
    '',
    'Сформуй загальний психологічний портрет у форматі JSON.',
    'У всіх текстових полях звертайся до людини на "Ви".',
    '',
    'Структура JSON:',
    '{',
    '  "type": "[1-3 слова — психологічний тип без кліше]",',
    '  "preview": "[Рівно 2 речення. Звернення на Ви. Перше — що Ви зараз переживаєте (конкретно). Друге — несподіване спостереження яке людина впізнає як правду про себе.]",',
    '  "full_text": "[4-5 речень. Звернення на Ви. Кожне речення — нова думка. Покажіть зв\'язки між сферами. Закінчити на ноті можливості — не проблеми.]",',
    '  "core_pattern": "[1 речення на Ви. Головний патерн який пояснює більшість низьких балів. Несподівано точно.]",',
    '  "growth_vector": "[1 речення на Ви. Конкретний напрямок — не розвивайтесь а що саме зробити першим.]",',
    '  "month_plan": [',
    '    {"week":1,"focus":"[Назва фокусу]","sphere":"[Сфера]","actions":["[Конкретна дія]","[Конкретна дія]","[Конкретна дія]"]},',
