...
function fixUkrainianText(text){
  ...
  let r=text;
  // --- ТАВТОЛОГІЯ "відчуваєте відчуття" ---
  // split/join — гарантовано працює з кирилицею (indexOf підтверджено)
  const taut=[
    ...
    ['Відчуваєте відчутну ','Помітно відчуваєте '],
    ['Відчуваєте відчутну','Помітно відчуваєте'],
    // Та ж логіка для відчуваєш / відчуває
    ['відчуваєш відчуття ','відчуваєш '],
    ['відчуваєш відчуття','відчуваєш'],
    ['відчуває відчуття ','відчуває '],
    ['відчуває відчуття','відчуває'],
  ];
  taut.forEach(([from,to])=>{ if(r.indexOf(from)!==-1) r=r.split(from).join(to); });
  // Загальний catch-all (lowercase і uppercase)
